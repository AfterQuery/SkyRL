...
#
# Harbor gives each trial its own container with its own seeded world, and -- because the
# mcp-atlas agent collects per-turn token IDs and logprobs -- step-wise training with
# off-policy correction (TIS) instead of re-tokenizing a finished conversation.
#
# Prerequisites:
